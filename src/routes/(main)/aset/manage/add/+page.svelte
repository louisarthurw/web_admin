<script>
	...
	export let data;
	// const userCompany = data.userCompany
...
>
	...
	>
		...
		<!-- <div>
			<label class="text-[#18294E] font-semibold" for="perusahaan_id">Company</label>
			<select
				name="perusahaan_id"
				class="w-full p-3 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#18294E] {selectedUserCompany ===
				''
					? 'text-gray-500'
					: 'text-black font-semibold'}"
				bind:value={selectedUserCompany}
				required
			>
				<option value="" class="text-gray-500" disabled selected>Choose Company</option>
				{#each userCompany as company}
					<option value={company.id_perusahaan} class="text-black font-semibold">{company.nama}</option>
				{/each}
			</select>
		</div> -->
